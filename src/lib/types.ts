    ...
    if (isManual) {
      if (!startTime || !endTime) {
        toast({
          ...
          title: 'Error de validación',
          description:
            'Por favor, ingrese hora de inicio y fin en modo manual.',
        });
        return;
      }
    } else {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl rounded-2xl">
        <DialogHeader>
          ...
          {step === 'form' && (
            <DialogDescription>
              Complete los datos para realizar el check-in.
            </DialogDescription>
          )}
        </DialogHeader>
        ...
        {step === 'form' ? (
            ...
            <div className="grid md:grid-cols-2 gap-3 py-2">
              ...
              <div className="grid gap-2 content-start">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant={entryType === 'Auto' ? 'default' : 'outline'}
                      onClick={() => setEntryType('Auto')}
                      className="flex-1 gap-2 text-xs h-9 px-2"
                    >
                      <Car /> Auto
                    </Button>
                    <Button
                      variant={entryType === 'Moto' ? 'default' : 'outline'}
                    ...
                    <Button
                      variant={entryType === 'Pie' ? 'default' : 'outline'}
              ...
              <div className="grid gap-2 content-start">
                <h3 className="font-semibold flex items-center gap-2 text-base">
                  <Calendar className="h-5 w-5" /> Tiempo de Estancia
                </h3>
                  ...
                  {isManual ? (
                    ...
                    <>
                      ...
                      <div className="rounded-lg border bg-background p-2 text-center">
                        ...
                        <div className="font-semibold text-base">
                          {endTime ? formatToMexicanTime(endTime) : '--:--'}
                        </div>
                      </div>
                    </>
                  )}
                    ...
                    {rates
                      ...
                      .map((rate, index) => (
                        <Button
                          key={rate.id}
                          variant={
                            selectedRate?.id === rate.id ? 'default' : 'outline'
                          }
                          className={cn(
                            'h-auto flex-col p-1 text-xs',
                            selectedRate?.id !== rate.id &&
                              rateColors[index % rateColors.length]
                          )}
            ...
            <div className="p-4 border-t">
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>
                    {selectedRate
                      ? `${selectedRate.name} - $${selectedRate.price}`
                      : 'Manual'}
                  </span>
                </div>
                <div className="flex justify-between font-bold text-foreground">
                  <span>Total:</span>{' '}
                  <span>
                    ${selectedRate ? selectedRate.price.toFixed(2) : '0.00'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="py-4 space-y-4">
            <h3 className="text-center font-headline text-lg">
              Confirmar Check-in
            </h3>
            <Card>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-3xl font-bold">
                      ${selectedRate?.price.toFixed(2) || '0.00'}
                    </p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Horas</Label>
                    <p className="text-3xl font-bold">
                      {selectedRate?.hours || 'Manual'}
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          {step === 'form' ? (
            <>
              ...
              <Button
                ...
                disabled={
                  !entryType ||
                  (isManual ? !startTime || !endTime : !selectedRate)
                }
              >
                Revisar y Confirmar
              </Button>
